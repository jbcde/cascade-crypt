...
type AriaEnc = Encryptor<Aria256>;
type AriaDec = Decryptor<Aria256>;
...
impl Cipher for AriaCipher {
    fn encrypt(&self, key: &[u8], plaintext: &[u8]) -> Result<Vec<u8>, CryptoError> {
        ...
        let cipher = AriaEnc::new_from_slices(key, &iv)
    ...
    fn decrypt(&self, key: &[u8], ciphertext: &[u8]) -> Result<Vec<u8>, CryptoError> {
        ...
        let cipher = AriaDec::new_from_slices(key, iv)
...
mod tests {
    ...
    fn test_aria_roundtrip() {
        let cipher = AriaCipher;
        let key = [0x42u8; 32];
        let plaintext = b"Hello, ARIA!";
