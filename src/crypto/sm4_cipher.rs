...
type Sm4Enc = Encryptor<Sm4>;
type Sm4Dec = Decryptor<Sm4>;

const IV_SIZE: usize = 16;
const BLOCK_SIZE: usize = 16;
const KEY_SIZE: usize = 16; // SM4 uses 128-bit key
...
impl Cipher for Sm4Cipher {
    fn encrypt(&self, key: &[u8], plaintext: &[u8]) -> Result<Vec<u8>, CryptoError> {
        ...
        let cipher = Sm4Enc::new_from_slices(&key[..KEY_SIZE], &iv)
    ...
    fn decrypt(&self, key: &[u8], ciphertext: &[u8]) -> Result<Vec<u8>, CryptoError> {
        ...
        let cipher = Sm4Dec::new_from_slices(&key[..KEY_SIZE], iv)
