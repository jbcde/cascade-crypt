...
type BlowfishEnc = Encryptor<Blowfish>;
type BlowfishDec = Decryptor<Blowfish>;
...
impl Cipher for BlowfishCipher {
    fn encrypt(&self, key: &[u8], plaintext: &[u8]) -> Result<Vec<u8>, CryptoError> {
        // Blowfish supports 32-448 bit keys, we use 256-bit (32 bytes)
        if key.len() != 32 {
        ...
        let cipher = BlowfishEnc::new_from_slices(key, &iv)
    ...
    fn decrypt(&self, key: &[u8], ciphertext: &[u8]) -> Result<Vec<u8>, CryptoError> {
        ...
        let cipher = BlowfishDec::new_from_slices(key, iv)
...
mod tests {
    ...
    fn test_blowfish_roundtrip() {
        let cipher = BlowfishCipher;
        let key = [0x42u8; 32];
        let plaintext = b"Hello, Blowfish!";
