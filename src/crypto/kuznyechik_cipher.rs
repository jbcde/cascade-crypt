...
type KuznyechikEnc = Encryptor<Kuznyechik>;
type KuznyechikDec = Decryptor<Kuznyechik>;
...
impl Cipher for KuznyechikCipher {
    fn encrypt(&self, key: &[u8], plaintext: &[u8]) -> Result<Vec<u8>, CryptoError> {
        ...
        let cipher = KuznyechikEnc::new_from_slices(key, &iv)
    ...
    fn decrypt(&self, key: &[u8], ciphertext: &[u8]) -> Result<Vec<u8>, CryptoError> {
        ...
        let cipher = KuznyechikDec::new_from_slices(key, iv)
...
mod tests {
    ...
    #[test]
    fn test_kuznyechik_roundtrip() {
        let cipher = KuznyechikCipher;
        let key = [0x42u8; 32];
        let plaintext = b"Hello, Kuznyechik!";
