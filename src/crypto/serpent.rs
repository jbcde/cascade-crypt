...
type SerpentEnc = Encryptor<Serpent>;
type SerpentDec = Decryptor<Serpent>;

const IV_SIZE: usize = 16; // 128-bit IV for Serpent
const BLOCK_SIZE: usize = 16; // 128-bit block for Serpent
...
impl Cipher for SerpentCipher {
    fn encrypt(&self, key: &[u8], plaintext: &[u8]) -> Result<Vec<u8>, CryptoError> {
        ...
        let cipher = SerpentEnc::new_from_slices(key, &iv)
    ...
    fn decrypt(&self, key: &[u8], ciphertext: &[u8]) -> Result<Vec<u8>, CryptoError> {
        ...
        let cipher = SerpentDec::new_from_slices(key, iv)
