...
type TwofishEnc = Encryptor<Twofish>;
type TwofishDec = Decryptor<Twofish>;

const IV_SIZE: usize = 16; // 128-bit IV for Twofish
const BLOCK_SIZE: usize = 16; // 128-bit block for Twofish
...
impl Cipher for TwofishCipher {
    fn encrypt(&self, key: &[u8], plaintext: &[u8]) -> Result<Vec<u8>, CryptoError> {
        ...
        let cipher = TwofishEnc::new_from_slices(key, &iv)
    ...
    fn decrypt(&self, key: &[u8], ciphertext: &[u8]) -> Result<Vec<u8>, CryptoError> {
        ...
        let cipher = TwofishDec::new_from_slices(key, iv)
