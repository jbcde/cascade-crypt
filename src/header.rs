...
impl Header {
    ...
    pub fn serialize(&self) -> String {
        ...
        format!(
            "[{}|{}|{}|{}|{}|{}|{}]\n",
            MAGIC, VERSION_PLAIN, self.algo_codes(), hex::encode(&self.salt), self.argon2_str(), ct_hash_hex, self.compute_hash()
        )
    }
    ...
    pub fn parse_encrypted<'a>(data: &'a [u8], private_key: &HybridPrivateKey) -> Result<(Self, &'a [u8]), HeaderError> {
        ...
        let ciphertext_hash = parse_hash(parts[5])?;
        Ok((Self { algorithms: parse_algos(&payload.algo_codes)?, salt: payload.salt, locked: payload.seal, ciphertext_hash: Some(ciphertext_hash), argon2_params: payload.argon2 }, remaining))
    }
...
fn parse_argon2(s: &str) -> Result<Argon2Params, HeaderError> {
    ...
    if parts.len() != 3 { return Err(HeaderError::InvalidFormat); }
    Ok(Argon2Params {
